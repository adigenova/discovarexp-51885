        ...
        vec<tuple<int,int,read_place>> edge_rid_place) 
{
    ...
    for (size_t i = 0; i < edge_rid_place.size(); i++) {
        ...
        tb << "REF extended" << Tab << String(-ext_len-min_shift, ' ') << ref_seg << EndRow;
        // the variant 
        tb << "ALT" << Tab << String(-min_shift, ' ') << bubble_graph.EdgeObject(e) << EndRow;
        tb << "REF" << Tab << String(-min_shift, ' ') << bubble_graph.EdgeObject(e_ref) << EndRow;
        // the reads
        for (size_t k = i; k < j; k++) {
            ...
            tb << sample << ":" << rid << (rp.Fw() ? "+":"-" ) 
                << " q " << rp.Qsum()/1000 << Tab;
            tb << String(shifts[k] - min_shift, ' ') << read << EndRow;
        }
        i = j - 1;
    }
    tb.Print(cout, 1, "ll");
...
//        if(verbosity>0) std::cout << "--end of group-- " << std::endl;
    ...
    if (verbosity >= 1){
        ...
        TextTable tb;
        tb << "VID" << Tab << "+Ref" << Tab << "-Ref" << Tab << "+Var" << Tab 
            << "-Var" << Tab << "Strand_Bias" << EndRow;
        tb << DoubleLine;
        for (size_t vid = 0; vid < vars.size(); vid++) {
...
*/
    ...
    #pragma omp parallel
    {
        ...
        for (size_t id = 0; id < PLACES2.size(); id++) 
