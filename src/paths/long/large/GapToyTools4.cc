          ...
          vec<triple<int,int,int>>& X, Bool debug = False )
{
     ...
     cout << "after kmers_plus creation, peak mem usage = " << PeakMemUsageGB( ) << " GB" << endl;
     ...
     const String& work_dir, const int EXT_MODE )
{
     ...
     if ( TRACE_PATHS.size() ) {
             ...
             {    const ReadPath& p = paths2[i];
                  cout << "[" << i  << "] (" << p.getOffset( ) << ") " << printSeq(p) //YYYY
                       << endl;
                  std::copy( p.begin(), p.end(), std::back_inserter(trace_edges) );
             }
     }
