...
template<int L> void ImprovePath( const vec< pair<int64_t, pair<int,int> > >& locsx,
     ...
     path_improver::path_status& status )
{
     ...
     vec<int> rstarts = {0, 20, 40, 60};      // read start positions for seeds
...
void ImprovePaths( ReadPathVec& paths, const HyperBasevector& hb,
     ...
     const Bool IMPROVE_PATHS_LARGE )
{
     // Build indices.
     ...
     {    path_improver::path_status status;
          ...
          ImprovePath( locsx, paths, id, paths[id], true_id, hb, inv, to_left, 
               to_right, bases[id], quals.begin()[id], kmers_plus, pimp, status );
