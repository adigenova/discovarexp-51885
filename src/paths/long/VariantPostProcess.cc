...
//                      if True in map( lambda x: x>0 and x<phred90, probs ):
                        if ( any_of(probs.begin(),probs.end(),[](double x){return x>0 && x<phred90;} )){
//                          new_samples = samples
                            new_samples = samples;
...
//                      calls = map( lambda x: x > phred995, probs )
                        ...
                        std::transform(probs.begin(),probs.end(),back_inserter(calls),[](double x){return (unsigned int)(x>phred995);});
